...
void Sample::Initialize(IUnknown* window, int width, int height, DXGI_MODE_ROTATION rotation)
{
	...
	m_ex = std::make_unique<SoundEffect>(m_audEngine.get(), L"explo1.wav");
	m_amb  = std::make_unique<SoundEffect>(m_audEngine.get(),L"NightAmbienceSimple_02.wav");
	// m_explode = m_soundEffect->CreateInstance();
		// m_ambient= m_soundEffect->CreateInstance();
}
